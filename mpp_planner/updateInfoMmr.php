<?php
    ...
    if ( null==$id ) {
        header("Location: mmrRequests.php");
    }
    ...
    if ( !empty($_POST)) {
        ...
        if ($valid) {
            ...
            $sql = "UPDATE mmrRequests set requestorName = ?, requestorDepartment = ?, requestorEmail = ?, requestStatus = ? WHERE requestId = ?";
            $q = $pdo->prepare($sql);
            $q->execute(array($name, $dept, $email, $requestStatus, $id));
        ...
        $pdo = Database::connect();
        $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $sql = "SELECT * FROM mmrRequests where requestId = ?";

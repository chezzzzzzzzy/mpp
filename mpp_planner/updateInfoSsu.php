...
if (!empty($_POST)) {
    ...
    if ($valid) {
        ...
        $sql = "UPDATE ssuRequests set requestorName = ?, requestorDepartment = ?, requestorEmail = ?, requestStatus = ?, numberOfPorts = ?, transmissionType = ?, interfacingType = ?, startDate = ?, endDate = ?, room = ? , exchange = ? WHERE requestId = ?";
        $q = $pdo->prepare($sql);
        $q->execute(array($name, $dept, $email, $requestStatus, $numberOfPorts, $transmissionType, $interfacingType, $startDate, $endDate, $room, $exchange, $id));

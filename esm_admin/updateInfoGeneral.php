<?php
    ...
    if ( !empty($_POST)) {
        ...
        if ($valid) {
            ...
            $sql = "UPDATE generalRequests set requestorName = ?, requestorDepartment = ?, requestorEmail = ?, requestStatus = ?, numberOfPorts = ?, numberOfCableTies = ?, completionDate = ?, room = ? , exchange = ? WHERE id = ?";
            $q = $pdo->prepare($sql);

<?php
    ...
    if ( !empty($_POST)) {
        ...
        if ($valid) {
            ...
            $sql = "UPDATE fdfRequests set requestorName = ?, requestorDepartment = ?, requestorEmail = ?, requestStatus = ?, numberOfPorts = ?, numberOfCableTies = ?, endDate = ?, room = ? , exchange = ? WHERE id = ?";
            $q = $pdo->prepare($sql);
            $q->execute(array($name, $dept, $email, $requestStatus, $numberOfPorts, $numberOfCableTies, $endDate, $room, $exchange, $id));
